...
- 背景冷色遮罩从深蓝黑改为浅蓝灰渐变：此前的深色遮罩会把背景压暗约 20%（下半屏更甚），现背景亮度接近原始图片，整体观感明显提亮，同时保留冷色氛围。
- 角色立绘投影减弱并增加方向性（模糊 30px→10px、透明度 0.45→0.25、向下偏移 18px）：消除立绘轮廓周围一圈的黑色淡晕（立绘去背残留边缘被深色大范围投影放大所致），发丝等细碎边缘不再显脏。

**后台日志清理**

- 独立 WebUI 代理不再记录每次请求的 debug 日志（此前后台日志会每 8 秒出现一条 `[proxy] GET .../config`，干扰其他日志查看），仅保留异常诊断日志。
- 页面切换至后台/隐藏时暂停 BGM 状态轮询，回到前台自动恢复——后台标签不再产生无效请求。

**代码维护**

- 后端结构重构：`api/session.py` 拆分（TTS 合成收敛至 `galgame_web/tts.py`、管道推送独立为 `pipeline.py`、音频工具独立为 `audio_utils.py`），消除 api 层对 `main.py` 的 23 处循环导入，路径常量统一至 `session_helpers.py` 单一真源。纯内部改动，无用户可见行为变化。
- 多角色对话功能开发中：角色卡数据层、会话模型多角色字段、消息编辑基础能力已就绪（面向后续版本）。

## v0.8.3

**Dashboard 内嵌页立绘修复 + 对话框简化**

- 修复内嵌页立绘消失（broken 占位符）：插件页 iframe sandbox 无 `allow-same-origin`，浏览器不发送 Dashboard 登录 cookie，立绘/背景直接 URL 加载全部 401。现内嵌页所有素材改为经认证 bridge 按需拉取 base64（立绘/背景/头像/预览均覆盖，单张懒加载 + 缓存，不回归性能）；独立 WebUI 保持 URL 直载快速路径。
- 对话框大幅简化：移除顶部黑条、纵向渐变、波浪/光点装饰、高光线，改为统一的半透明深藏青底（68% 透明度 + 轻模糊 + 1px 细边 + 小圆角），简洁通透，更接近视觉小说文本框。
- 输入行与对话框同款半透明底；历史面板背景、背景取色等在 Dashboard 下改用缓存优先，避免无效请求与闪烁。

## v0.8.2

**布局级视觉小说化（第二轮 UI）**

- 对话框重做：深蓝半透明 + 高斯模糊（backdrop-filter blur 10px），白色高对比文字，顶部 1px 冷蓝渐变装饰线，加大内边距与行距——背景细节不再干扰文字。
- 输入框移至对话框底部（符合视觉小说/聊天直觉），占位文字去掉技术提示（Enter 提示移入悬浮 title）。
- 背景统一冷调：新增上淡下深的冷色渐变遮罩，任意色调的背景图（如暖色房间场景）都被统一到冷调氛围，对话框区域自然加深保证可读性。
- 角色立绘增加冷蓝投影（drop-shadow），与背景分离不再像贴纸。
- 角色名标签精致化：深蓝底 + 浅蓝字 + 1.5px 字距 + 细边框。
- 顶部导航按钮：深蓝半透明底 + 细边框 + 浅色图标。
- 可访问性：全局 `:focus-visible` 焦点环；`prefers-reduced-motion` 下关闭打字机/呼吸等动画。
- 设置页卡片间距收紧（28px → 20px）。
- 视觉验证：无头浏览器截图 + 多模态模型审查（改版前 vs 改版后对比），确认"像视觉小说界面"。

## v0.8.1

**视觉小说风格界面重设计（冷色基调）**

- 主对话界面全面换肤：冷白半透明对话框 + 深蓝灰文字 + 细边框小圆角，去除紫色毛玻璃与发光效果，更接近日系视觉小说的界面气质。
- 历史/会话/设置/收藏/确认弹层/toast/独立 WebUI 登录页统一为深蓝灰 + 浅蓝灰文字的中性配色。
- 字体系统：标题默认衬线体（宋体类）、正文黑体，设置页新增「字体」选项（默认/全黑体/系统默认），保存后主页面即时生效，双端共享同一偏好。
- 收藏页「取消收藏」现在会弹出确认（此前为一键删除）。
- 布局零改动，仅皮肤层替换。
...
- 自绘确认弹层（`showConfirm`）替换原生 `confirm()`——原生确认框在 Dashboard 内嵌页 iframe sandbox（无 allow-modals）中静默失效，此前全部删除操作一键直删无确认。
- 四项删除操作现在都会弹出确认：会话删除（提示 AstrBot 对话记录与关联语音将一并清除）、素材单删、素材批量删、BGM 删除。独立 WebUI 与 Dashboard 内嵌页行为完全一致。
- 收藏页「取消收藏」保持一键操作（双端一致），不再使用原生 confirm。
- 自绘 toast 替换全部原生 `alert()`：内嵌页错误提示不再静默消失（素材/BGM/收藏删除失败、会话删除失败均有可见反馈）；BGM 删除成功增加成功提示。
- 主页面快速点击检测排除确认弹层区域，点击确认/取消/按 Esc 不会误触发「AI 主动关心」。
- 全项目零原生 `confirm()`/`alert()` 残留。

## v0.7.14

**运行可靠性与 WebUI 体验修复**

- 修复快速点击状态未被消费的问题；达到阈值后仅发送一次 `(戳了戳)`，失败或空回复也会恢复输入框。
- 保留“新对话优先复用空白会话”逻辑，并修复生成回复期间误复用正在使用的空白会话、连续点击重复创建及会话切换重复绑定事件的问题。
- 语音改为引用感知回收：仍在任一会话历史中或已收藏的文件会保留，仅在最后一个历史/收藏引用消失后删除。
- 插件卸载时统一取消并等待后台任务，修复重载后旧任务继续写入会话、TTS 或 AstrBot 数据的问题。
- 多句 TTS 可在无 ffmpeg 时直接合并兼容 WAV；部分分句失败或格式无法合并时改为整段重试，不再静默只播放第一句。
- TTS provider 返回的临时音频会在持久化后回收，并补齐 AAC、Opus 等音频格式的扩展名与 MIME 识别。
- 修复 AstrBot 平台消息历史的会话索引和消息段格式，并将偏好、收藏与会话数据统一改为原子写入。
- Dashboard 设置页补回全选，内外设置页支持拖拽批量上传；素材预览按视口分批加载，减少大量立绘时的内存和传输开销。
- 修复切换会话时旧打字机、语音和收藏状态残留，补充移动端对话、设置及收藏布局和基础无障碍标注。
- 新增运行可靠性回归测试；当前安全、会话、音频、TTS 和前端测试共 64 项。

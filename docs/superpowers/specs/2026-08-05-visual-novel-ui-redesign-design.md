...
日期：2026-08-05
状态：设计阶段（用户已确认方向：视觉小说冷色调、浮动卡片微调、面板深蓝灰、标题衬线+正文黑体+可自定义字体、收藏页取消收藏加确认）
目标版本：v0.8.1

## 1. 背景

用户反馈当前 WebUI "紫紫的一看就是 AI 干的"。期望向前翼社日系视觉小说的界面气质靠拢：浅色半透明对话框、克制的小圆角与细边框、无浓毛玻璃/霓虹/发光，管理面板采用游戏系统菜单式的深色中性配色。
...
| `backdrop-filter: blur()` 玻璃感与 视觉小说"克制"冲突 | 对话框/角色名/按钮/输入行/面板全部移除 `backdrop-filter`（深蓝灰面板 0.90 不透明度已足够） |

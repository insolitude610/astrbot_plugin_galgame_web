...
你可以在 AstrBot 配置页的表情配置项中**自由增删情绪 key**，添加如 `dokidoki`、`cry`、`smirk` 等任意情绪。AI 的 system prompt 会自动包含所有已定义的情绪标签。

> 注意：新增情绪后需要上传对应的表情图，否则该情绪出现时角色脸会变空。
...
- **立绘管理页按模式分区** —— 分别展示 single / layered 模式所需文件清单
- **槽位上传自动重命名** —— 从指定槽位上传的文件自动改为标准名
- **支持自定义情绪** —— expressions 配置改为 dict 类型，用户可自由增删情绪 key
- 详细说明两种模式的文件需求差异（single=全身 / layered=仅脸部 + 图层）

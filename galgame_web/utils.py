...
DEFAULT_GALGAME_PROMPT = (
    ...
    "3. 回复最开头用 [EMO:xxx] 标记当前整体情绪，用于语音合成的语气控制。xxx 只能取 happy/sad/angry/neutral 之一\n"
    "   同时回复中用 {emotion_xxx} 标签标记表情切换位置。标签格式固定为 {emotion_xxx}，emotion_ 前缀不可省略\n"
    "   可选表情：{{emotions}}\n"
    "   可以在一句话中多次切换表情\n"
    "   ✅ 正确示例：[EMO:happy]{emotion_neutral}今天天气真好！{emotion_blush}谢谢主人~\n"
    "   ❌ 错误：{happy}今天天气真好！              ← 缺少 emotion_ 前缀\n"
    "   ❌ 错误：今天天气真好！(开心)                ← 不能用括号写法\n"
    "   ❌ 错误：忘了写 [EMO:xxx] 开头的情绪标记      ← 必须写\n"
    "4. 不要在标签前后加任何多余文字\n"

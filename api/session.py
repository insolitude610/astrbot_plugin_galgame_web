...
class SessionAPI:
    def _register_session_apis(self):
        ...
        self.context.register_web_api(f"/{pn}/session/init", self._api_session_init, ["POST"], "Initialize or resume a galgame session")
        self.context.register_web_api(f"/{pn}/send", self._api_send, ["POST"], "Send a user message to the AI character")
        self.context.register_web_api(f"/{pn}/history", self._api_history, ["GET"], "Get conversation history")
        self.context.register_web_api(f"/{pn}/session/list", self._api_session_list, ["GET"], "List available sessions for recovery")
        self.context.register_web_api(f"/{pn}/session/delete", self._api_session_delete, ["POST"], "Delete a session and its AstrBot conversation")
        self.context.register_web_api(f"/{pn}/rapid_action", self._api_rapid_action, ["POST"], "Notify rapid click activity")
    ...
    async def _api_session_init(self):
        ...
        try:
            ...
            if resume_id and resume_id in self._sessions:
                return {"session_id": resume_id, "current_emotion": self._sessions[resume_id].get("current_emotion", "neutral")}
    ...
    async def _api_session_delete(self):
        from ..galgame_web.session_helpers import delete_astrbot_conv, cleanup_session_audio, session_path
        data = await request.get_json() or {}
                ...
                if mtype == "end":
                    ...
                    if record_file:
                        ...
                        if record_path.exists():
                            ...
                            audio_file = f"{uuid.uuid4().hex}{self._ext_for_mime(audio_mime)}"
    ...
    def _send_handle_command_result(self, cmd, session, sid, raw_reply, pipeline_result):
        async def _sync():
            if cmd == "new":
    ...
    def _send_save_and_return(self, session, text, clean_text, emotions, emotions_all,
        ...
        try:
            async def _save_history():
                await self.context.message_history_manager.insert(
                    platform_id=PLATFORM_ID, user_id=conv_id,
                    content={"type": "user", "message": text}, sender_id="user", sender_name="用户")
                await self.context.message_history_manager.insert(
                    platform_id=PLATFORM_ID, user_id=conv_id,
                    content={"type": "bot", "message": clean_text}, sender_id="bot", sender_name=character_name)
            import asyncio
        ...
        return {"reply": clean_text, "emotion": final_emotion,
                "emotions": [[emo, pos] for emo, pos in emotions],
                "audio": audio_b64, "audio_mime": audio_mime_val,
                "audio_file": audio_file, "audio_segments": []}
